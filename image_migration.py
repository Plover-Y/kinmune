...
def get_images(aifrom=None):
    ...
    decoded_json = session.get('http://'+wiki+'/api.php', params=payload, headers=headers).json()

    for image in decoded_json['query']['allimages']:
        # get page contents
        payload = {'action': 'raw'}
        body = session.get('http://'+wiki+'/wiki/'+image['title'], params=payload).text
        ...
        result = session.post('https://'+dest_wiki+'/api.php', data=payload, headers=headers, verify=False)
        print(result.text)
